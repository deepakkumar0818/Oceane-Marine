...
export default function LocationsPage() {
  ...
  const activeTab = sidebarTabs.find((tab) => pathname === tab.href)?.key || "locations";
  ...
  return (
    <div className="min-h-screen bg-transparent text-white flex">
            ...
            <div className="space-y-2">
              {sidebarTabs.map((tab) => (
                <Link
                  key={tab.key}
                  href={tab.href}
                  className={`block w-full text-left px-4 py-3 rounded-xl text-sm font-medium transition-all duration-150 ${activeTab === tab.key ? "bg-orange-500 text-white shadow-lg shadow-orange-500/40" : "text-white/90 hover:bg-white/10 hover:text-white border border-white/5"}`}
                >
                  {tab.label}
                </Link>
              ))}
      ...
      {!isSidebarOpen && (
        <button onClick={() => setIsSidebarOpen(true)} className="fixed left-4 top-4 z-40 flex h-10 w-10 items-center justify-center rounded-xl bg-white/10 hover:bg-white/20 transition border border-white/10 shadow-lg" aria-label="Open sidebar">
