...
export async function GET(req) {
  ...
  try {
    ...
    const reports = await MooringMastersJobReport.find(query)
      .populate("createdBy", "name email")
      .sort({ createdAt: -1 })
      .lean();

    // Get available years
    const allReports = await MooringMastersJobReport.find({
    ...
    allReports.forEach((report) => {
      if (report.documentInfo?.issueDate) {
        const reportYear = new Date(report.documentInfo.issueDate).getFullYear();
    ...
    console.error("OPS-OFD-009 list error:", error);
    return NextResponse.json(
      {
        success: false,
        error: error.message,
