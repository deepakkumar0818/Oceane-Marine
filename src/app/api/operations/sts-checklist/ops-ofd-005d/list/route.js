...
export async function GET(req) {
  ...
  try {
    ...
    const checklists = await STSChecklist5D.find(query)
      .populate("createdBy", "name email")
      .sort({ createdAt: -1 })
      .lean();

    const allChecklists = await STSChecklist5D.find({
    ...
    allChecklists.forEach((checklist) => {
      if (checklist.documentInfo?.issueDate) {
        const checklistYear = new Date(checklist.documentInfo.issueDate).getFullYear();
    ...
    console.error("OPS-OFD-005D list error:", error);
    return NextResponse.json(
      {
        success: false,
        error: error.message,
